...
$shellCommand = "for i in 1 2 3 4 5; do iptables -t nat -D OUTPUT $ruleSpec >/dev/null 2>&1 || break; done; iptables -t nat -A OUTPUT $ruleSpec; iptables -t nat -S OUTPUT"
...
Write-Host "Applying redirect: $TargetHost`:$TargetPort -> $RedirectHost`:$RedirectPort"
